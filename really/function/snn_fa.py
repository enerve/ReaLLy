...
class SNN_FA(ValueFunction):
    ...
    def compute_results(self, queries):
        X = self.feature_eng.inputs_for_queries(queries)
        self.net.eval()
        with torch.no_grad():
            outputs = self.net(X)
        return self.feature_eng.values_from_outputs(outputs)
...
#         return self.feature_eng.value_from_output(output)
    ...
    def best_action(self, state):
        self.compute_results(self.feature_eng.available_queries)
